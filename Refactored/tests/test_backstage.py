...
def test_backstage_quality_increases_by_one_when_sell_in_higher_than_10():
    items = [Item(BACKSTAGE, 12, 10)]
    update_quality_list(items)
    assert 11 == items[0].quality

def test_backstage_quality_increases_by_two_when_sell_in_between_10_and_5_days():
    items = [Item(BACKSTAGE, 10, 10)]
    update_quality_list(items)
    assert 12 == items[0].quality

def test_backstage_quality_increases_by_three_when_sell_in_between_5_and_0_days():
    items = [Item(BACKSTAGE, 5, 10)]
    update_quality_list(items)
    assert 13 == items[0].quality

def test_backstage_quality_drops_to_zero_after_sell_in_date():
